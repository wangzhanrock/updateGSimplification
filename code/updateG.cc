...
// REMARK: What you do here, you could do that with every object. The objects in the CVT will then return 3x3 matrix,
// where you will check for non-zero values also. Why not? Is more general and I guess does not really hurt, right?
// Maybe it would also be possible to give every object the possibility to return the non-zero values in a sparse format and
// return it here... So we could also avoid the checking every time-step.

  int n = LLM[j].cols();

// REMARK: the compressLLM should be a function of the DS, as the knowledge about the objects is there already.
// It must not be implemented in a CVT-specific way, as I also don't really understand what is the profit out of it
// REMARK: nz could be guessed by the sub-block-sizes!
  cs_L_LLM = compressLLM_LToCsparse(t, j);
...
//    free(x);
  ...
  // todo: do we need sparse matrix multiplication for G = y'*y, when G is a dense matrix. check by matlab
  // the matlab test shows that with full matrix format is faster, this is because that y is stored in compressed-column format and it is hard to access the the column of y'.
...
//  Gs.resize(y.cols(), y.cols() * y.cols());  // todo: refine the size of nonzero value
//  double EPSILON = 1e-17;
//  double *Gsele = Gs.operator()();
//  int *GsI= Gs.Ip();
//  int *GsJ = Gs.Jp();
//  int counter = 0;
//
...
//    Gsele[counter] = scalarProduct(temp, temp);
//    GsJ[counter] = i;
//    GsI[i] = counter++;
//    std::vector<double> nzInRow;
//    for (int j = 0; j < y.cols(); j++) {
//      double val = scalarProduct(temp, y.col(j));
//      if (fabs(val) > EPSILON){
//        nzInRow.push_back(val);
//        Gsele[counter] = val;
//        GsJ[counter++] = j;
//      }
//    }
//  }
//  if(y.cols()) GsI[y.cols()]=counter;
